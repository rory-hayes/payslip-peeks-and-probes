...
6. Disable Supabase email auto-confirm so every new account proves control of its email address, configure the production web and native Auth redirects, then test sign-up, confirmation and password recovery. Native auth uses PKCE and rejects bearer tokens from custom-scheme callbacks; add verified HTTPS Universal Links/App Links before a store release.
...
13. Configure usable MX mail routing for `payslipinsights.com`, route both `support@payslipinsights.com` and `privacy@payslipinsights.com` to monitored inboxes, and prove inbound delivery plus a human reply from each address. Keep the destination inbox private; only the public role addresses belong in source or release output.
14. Choose and verify the production hosting/domain path for Expo web before pointing `payslipinsights.com` at it.
...
It verifies the live `release.json`, title, production surface, clean-worktree receipt, cache revalidation, the built application module, basic protective headers, usable MX routing for the published support/privacy domain, and absence of the host editing badge or host-injected analytics. It also confirms that `/guides`, one guide, and `/pricing` serve their own pre-hydration canonical, social, and structured-data metadata rather than the homepage shell, while `/sign-in` and `/dashboard` serve static noindex HTML. It deliberately does not prove delivery to the destination inbox, upload a document, sign in, or exercise Stripe; complete the operational-mail, staged provider, and two-account checks above separately.
