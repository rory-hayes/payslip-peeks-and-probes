...
describe('Landing', () => {
  ...
  it('keeps the selected consumer journey wired to sign-up, demo, and anchored sections', () => {
    ...
    expect(screen.getByText('UK or Ireland tax-year action plan and records checklist')).toBeInTheDocument();
    ...
    expect(screen.getByRole('heading', { name: 'A tax-year plan built on official sources' })).toBeInTheDocument();
    expect(screen.getByRole('article', { name: 'Sample official-source tax-year action plan' })).toHaveTextContent('Official service makes the decision');
    expect(screen.getByText(/billing follows your chosen country: Ireland in EUR, United Kingdom in GBP/i)).toBeInTheDocument();
  ...
  it('opens the sample tax-year helper without asking a visitor to create an account', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Explore the tax-year planner' }));
