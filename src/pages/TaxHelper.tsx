...
import { ArrowRight, Check, ExternalLink, FileCheck2, Landmark, ShieldCheck } from 'lucide-react';
...
import { isDateInTaxYear, OFFICIAL_TAX_STEPS, taxYearWindow, type TaxHelperCountry } from '@/lib/tax-helper';
      ...
      <div className="tax-helper">
        ...
        <div className="tax-helper__controls">
          <div className="tax-helper__country" role="group" aria-label="Choose tax country">
            <button aria-pressed={country === 'Ireland'} onClick={() => { setManualCountry('Ireland'); setCompleted(new Set()); }} type="button">Ireland</button>
            <button aria-pressed={country === 'UK'} onClick={() => { setManualCountry('UK'); setCompleted(new Set()); }} type="button">United Kingdom</button>
          </div>
          <div className="tax-helper__country" role="group" aria-label="Choose tax-year period">
            <button aria-pressed={period === 'completed'} onClick={() => { setPeriod('completed'); setCompleted(new Set()); }} type="button">Last completed</button>
            <button aria-pressed={period === 'current'} onClick={() => { setPeriod('current'); setCompleted(new Set()); }} type="button">Current year</button>
          </div>
        </div>
        ...
        <section className="tax-helper__steps" aria-labelledby="tax-steps-heading">
          <div className="tax-helper__section-heading">
            <div>
              ...
              <h2 id="tax-steps-heading">Your {window.label} review</h2>
            </div>
